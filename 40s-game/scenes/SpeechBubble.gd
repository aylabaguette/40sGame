...
func showSpeechBubble():
	...
	#gest like the array from the other script and duplicates it and makes a random order
	var toppings_array = SignalBus.unlockedToppings.duplicate()
	toppings_array.shuffle()
	
	#gets the first and second toppings within the suffled duplicated array
	var toppingSelected = toppings_array[0]
	var toppingSelected2 = toppings_array[1]
	
	#gets the array from the other script and duplicates it and puts it in a random order
	var drinks_array = SignalBus.unlockedDrinks.duplicate()
	drinks_array.shuffle()
	
	#gets the first flavour from the shuffled array
